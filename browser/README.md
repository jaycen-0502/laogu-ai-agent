...
> 本目录是 `laogu-ai-agent` 私有仓库中的 Browser 源码组件。运行数据、实例资料、登录状态、
> Cookie、真实配置、授权私钥和编译产物均被排除；构建说明见
> [BUNDLE_BUILD_ZH_CN.md](BUNDLE_BUILD_ZH_CN.md)。
...
Laogu Browser 当前推荐配套使用的浏览器内核，来源于开源项目 [fingerprint-chromium](https://github.com/adryfish/fingerprint-chromium)。

如果你正在寻找可直接下载和维护的指纹内核版本，建议先查看它的 Releases 页面：

- <https://github.com/adryfish/fingerprint-chromium/releases>

这个项目为 Laogu Browser 的内核准备提供了直接可用的基础来源，这里先对原项目做明确推荐与致谢。

Laogu Browser 的目标很明确：在一台桌面设备上，帮助用户稳定管理多个彼此隔离的浏览器实例，并配合代理池、浏览器内核和快捷启动能力完成日常运营或测试工作。
...
- 自动化增强：完善自动化程序导入、运行、目标实例选择和执行记录管理，提升多实例自动化编排能力
...
- 自动化程序：支持脚本导入、运行、目标实例选择、执行记录和外部接口调用
...
如果代理池里本地客户端可用节点很多，但 Laogu Browser 中“只展示可用”数量明显偏少，先确认当前 `default_connector_type` 是否与本地客户端一致。Laogu Browser 不会在 `xray` 组合栈和 `mihomo` 栈之间自动混用；切换连接栈后需要重新测速。
