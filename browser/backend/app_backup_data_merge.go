...
WHERE NOT EXISTS (
  ...
  WHERE t.profile_id = s.profile_id OR lower(t.user_data_dir) = lower(s.user_data_dir)
		...
		{
			name: "automation_account_states",
			insertAll: `INSERT INTO automation_account_states (profile_id, platform, script_id, keywords_json, cursor_json, created_at, updated_at)
SELECT profile_id, platform, script_id, keywords_json, cursor_json, created_at, updated_at FROM src.automation_account_states`,
			insertSafe: `INSERT INTO automation_account_states (profile_id, platform, script_id, keywords_json, cursor_json, created_at, updated_at)
SELECT s.profile_id, s.platform, s.script_id, s.keywords_json, s.cursor_json, s.created_at, s.updated_at
FROM src.automation_account_states s
WHERE EXISTS (SELECT 1 FROM browser_profiles p WHERE p.profile_id = s.profile_id)
  AND NOT EXISTS (
    SELECT 1 FROM automation_account_states t
    WHERE t.profile_id = s.profile_id AND t.platform = s.platform AND t.script_id = s.script_id
  )`,
		},
		{
			name: "automation_processed_items",
			insertAll: `INSERT INTO automation_processed_items (profile_id, platform, item_type, item_key, metadata_json, processed_at)
SELECT profile_id, platform, item_type, item_key, metadata_json, processed_at FROM src.automation_processed_items`,
			insertSafe: `INSERT INTO automation_processed_items (profile_id, platform, item_type, item_key, metadata_json, processed_at)
SELECT s.profile_id, s.platform, s.item_type, s.item_key, s.metadata_json, s.processed_at
FROM src.automation_processed_items s
WHERE EXISTS (SELECT 1 FROM browser_profiles p WHERE p.profile_id = s.profile_id)
  AND NOT EXISTS (
    SELECT 1 FROM automation_processed_items t
    WHERE t.profile_id = s.profile_id AND t.platform = s.platform AND t.item_type = s.item_type AND t.item_key = s.item_key
  )`,
		},
		{
			name: "automation_daily_counters",
			insertAll: `INSERT INTO automation_daily_counters (profile_id, platform, counter_date, counter_key, value, updated_at)
SELECT profile_id, platform, counter_date, counter_key, value, updated_at FROM src.automation_daily_counters`,
			insertSafe: `INSERT INTO automation_daily_counters (profile_id, platform, counter_date, counter_key, value, updated_at)
SELECT s.profile_id, s.platform, s.counter_date, s.counter_key, s.value, s.updated_at
FROM src.automation_daily_counters s
WHERE EXISTS (SELECT 1 FROM browser_profiles p WHERE p.profile_id = s.profile_id)
  AND NOT EXISTS (
    SELECT 1 FROM automation_daily_counters t
    WHERE t.profile_id = s.profile_id AND t.platform = s.platform AND t.counter_date = s.counter_date AND t.counter_key = s.counter_key
  )`,
		},
		{
