...
func TestBuildBrowserFingerprintLaunchPlanChrome144RemovesNoEffectFineGrainedArgs(t *testing.T) {
	...
	assertRowsContainStatus(t, plan.rows, "字体", "host_value")
}

func TestBuildBrowserFingerprintLaunchPlanAlwaysUsesHostFonts(t *testing.T) {
	for _, version := range []string{"143.0.7499.10", ""} {
		plan := buildBrowserFingerprintLaunchPlan("profile-a", []string{
			"--fingerprint=123",
			"--fingerprint-font-list=Arial,Calibri",
			"--fingerprint-fonts=Arial,Calibri",
		}, version)

		assertArgsDoNotContainKey(t, plan.launchArgs, "--fingerprint-font-list")
		assertArgsDoNotContainKey(t, plan.launchArgs, "--fingerprint-fonts")
		assertRowsContainStatus(t, plan.rows, "字体", "host_value")
	}
}
