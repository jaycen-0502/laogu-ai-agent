...
func (a *App) startup(ctx context.Context) {
	...
	if err := a.ensureLicenseManager(); err != nil {
		runtime.LogError(ctx, fmt.Sprintf("初始化离线授权失败: %v", err))
		return
	}
	if serverURL := a.licenseServerURL(); serverURL != "" {
		status, err := a.licenseMgr.RemoteCheck(serverURL)
		if err != nil && !status.Licensed {
			log.Printf("remote license check failed: %v", err)
			runtime.LogWarning(ctx, fmt.Sprintf("Remote license check unavailable: %v", err))
			return
		}
	}
	if !a.licenseMgr.Status().Licensed {
		return
	}
	a.startupLicensed(ctx)
}

func (a *App) startupLicensed(ctx context.Context) {
	a.startupMu.Lock()
...
func (a *App) startupInitManagers(cfg *config.Config, db *database.DB) {
	...
	conn := db.GetConn()
	a.automationStateStore = automationstate.NewStore(conn)
...
func (a *App) startupInitLaunchServer(log *logger.Logger) {
	...
	a.launchServer.SetAutomationStateProvider(a.automationStateStore)
