...
func (m *Manager) Delete(profileId string) error {
	...
	if err := m.deleteProfileFingerprintCheckCacheDir(profile.ProfileId); err != nil {
		log.Warn("清理实例指纹检测缓存失败", logger.F("profile_id", profileId), logger.F("error", err.Error()))
	}
...
func (m *Manager) ListDeleted() []Profile {
	...
	_ = m.cleanupExpiredTrashLocked(log)
	if m.ProfileDAO == nil {
		return []Profile{}
	}
...
func (m *Manager) deleteProfileRelatedDataLocked(log *logger.Logger, profile *Profile) error {
	...
	if err := m.deleteProfileFingerprintCheckCacheDir(profile.ProfileId); err != nil {
		log.Error("删除实例指纹检测缓存失败", logger.F("profile_id", profile.ProfileId), logger.F("error", err))
		if firstErr == nil {
			firstErr = err
		}
	}
...
func (m *Manager) deleteProfileFingerprintCheckCacheDir(profileId string) error {
	if m == nil {
		return nil
	}
	dataRoot, err := filepath.Abs(m.ResolveRelativePath("data"))
	if err != nil {
		return fmt.Errorf("解析指纹检测数据根目录失败: %w", err)
	}
	cacheRoot := filepath.Clean(filepath.Join(dataRoot, "fingerprint-check"))
	target := filepath.Clean(filepath.Join(cacheRoot, safeProfileCachePath(profileId)))
	if samePath(target, dataRoot) || samePath(target, cacheRoot) || !isPathInside(target, cacheRoot) {
		return nil
	}
	if err := os.RemoveAll(target); err != nil {
		return fmt.Errorf("删除指纹检测缓存目录失败: %w", err)
	}
	return nil
}

func safeProfileCachePath(profileId string) string {
