...
func TestPermanentlyDeleteRemovesUserDataDirAndWritesAudit(t *testing.T) {
	...
	fingerprintCacheDir := filepath.Join(appRoot, "data", "fingerprint-check", profile.ProfileId)
	if err := os.MkdirAll(fingerprintCacheDir, 0755); err != nil {
		t.Fatalf("MkdirAll fingerprint cache returned error: %v", err)
	}
	if err := os.WriteFile(filepath.Join(fingerprintCacheDir, "index.html"), []byte("cache"), 0644); err != nil {
		t.Fatalf("WriteFile fingerprint cache returned error: %v", err)
	}
	...
	if _, err := os.Stat(fingerprintCacheDir); !os.IsNotExist(err) {
		t.Fatalf("fingerprint cache dir still exists after permanent delete: %v", err)
	}
