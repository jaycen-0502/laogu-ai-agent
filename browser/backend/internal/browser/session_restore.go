...
func ClearSessionRestoreData(userDataDir string) error {
	...
	if _, statErr := os.Stat(sessionsDir); statErr == nil {
		if err := os.RemoveAll(sessionsDir); err != nil && !errors.Is(err, os.ErrNotExist) {
	...
	for _, name := range sessionRestoreLegacyFiles {
		if err := os.Remove(filepath.Join(profileDir, name)); err != nil && !errors.Is(err, os.ErrNotExist) {
