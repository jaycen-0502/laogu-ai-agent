...
var migrations = []migration{
	...
	{
		version: 13,
		desc:    "添加账号自动化状态、去重记录和每日计数",
		stmts: []string{
			`CREATE TABLE IF NOT EXISTS automation_account_states (
				profile_id    TEXT NOT NULL,
				platform      TEXT NOT NULL DEFAULT 'generic',
				script_id     TEXT NOT NULL DEFAULT 'default',
				keywords_json TEXT NOT NULL DEFAULT '[]',
				cursor_json   TEXT NOT NULL DEFAULT '{}',
				created_at    TEXT NOT NULL DEFAULT '',
				updated_at    TEXT NOT NULL DEFAULT '',
				PRIMARY KEY (profile_id, platform, script_id),
				FOREIGN KEY (profile_id) REFERENCES browser_profiles(profile_id) ON DELETE CASCADE
			)`,
			`CREATE INDEX IF NOT EXISTS idx_automation_account_states_platform ON automation_account_states(platform, script_id)`,
			`CREATE TABLE IF NOT EXISTS automation_processed_items (
				profile_id    TEXT NOT NULL,
				platform      TEXT NOT NULL DEFAULT 'generic',
				...
				PRIMARY KEY (profile_id, platform, item_type, item_key),
				FOREIGN KEY (profile_id) REFERENCES browser_profiles(profile_id) ON DELETE CASCADE
			)`,
			`CREATE INDEX IF NOT EXISTS idx_automation_processed_items_time ON automation_processed_items(profile_id, platform, processed_at)`,
			`CREATE TABLE IF NOT EXISTS automation_daily_counters (
				profile_id   TEXT NOT NULL,
				platform     TEXT NOT NULL DEFAULT 'generic',
				...
				updated_at   TEXT NOT NULL DEFAULT '',
				PRIMARY KEY (profile_id, platform, counter_date, counter_key),
				FOREIGN KEY (profile_id) REFERENCES browser_profiles(profile_id) ON DELETE CASCADE
			)`,
			`CREATE INDEX IF NOT EXISTS idx_automation_daily_counters_profile_date ON automation_daily_counters(profile_id, platform, counter_date)`,
		},
	},
	// ── 新版本在此追加，格式：
	// {
