...
type automationScriptRunAPIRequest struct {
	...
	ProfileID         string          `json:"profileId"` // 允许接收 Profile ID
	TaskKey           string          `json:"taskKey"`   // 允许接收 TaskKey
...
func (s *LaunchServer) handleAutomationScriptRun(w http.ResponseWriter, r *http.Request) {
	...
	// 放宽请求体校验，避免因未定义的额外扩展字段（如 profileId/taskKey）导致解码直接被挂断
	// dec.DisallowUnknownFields()

	if err := dec.Decode(&req); err != nil {
		writeAutomationAPIError(w, http.StatusBadRequest, "invalid_request", "invalid request body: "+err.Error(), "")
		return
	}
...
func normalizeAutomationRunRequest(req automationScriptRunAPIRequest) (automation.ScriptRunRequest, error) {
	...
	// 如果 Params 中没有显式注入 profileId，优先提取顶层请求中的 profileId/taskKey 进行补全
	if params == nil {
		params = make(map[string]interface{})
	}
	if _, ok := params["profileId"]; !ok && req.ProfileID != "" {
		params["profileId"] = req.ProfileID
	}
	if _, ok := params["taskKey"]; !ok && req.TaskKey != "" {
		params["taskKey"] = req.TaskKey
	}
