...
// AutomationScriptRunLister 可选接口：提供自动化程序运行记录。
type AutomationScriptRunLister interface {
	AutomationScriptRunList(limit int) ([]automation.ScriptRunRecord, error)
}

type AutomationStateProvider interface {
	GetState(profileID, platform, scriptID string) (automationstate.AccountState, error)
	SaveKeywords(profileID, platform, scriptID string, keywords []string) (automationstate.AccountState, error)
	SaveCursor(profileID, platform, scriptID string, cursor map[string]interface{}) (automationstate.AccountState, error)
	IsProcessed(input automationstate.ProcessedItemInput) (automationstate.ProcessedItemStatus, error)
	MarkProcessed(input automationstate.ProcessedItemInput) (automationstate.ProcessedItemStatus, error)
	IncrementCounter(input automationstate.CounterIncrementInput) (automationstate.DailyCounter, error)
	ListCounters(profileID, platform, counterDate string) ([]automationstate.DailyCounter, error)
}
...
type LaunchServer struct {
	...
	automationState AutomationStateProvider
}

func (s *LaunchServer) SetAutomationStateProvider(provider AutomationStateProvider) {
	s.mu.Lock()
	s.automationState = provider
	s.mu.Unlock()
}

func (s *LaunchServer) automationStateProvider() AutomationStateProvider {
	s.mu.Lock()
	defer s.mu.Unlock()
	return s.automationState
}
