...
func (s *LaunchServer) buildMux() *http.ServeMux {
	...
	mux.HandleFunc("/api/automation/account-state", s.handleAutomationAccountState)
	mux.HandleFunc("/api/automation/processed/check", s.handleAutomationProcessedCheck)
	mux.HandleFunc("/api/automation/processed/mark", s.handleAutomationProcessedMark)
	mux.HandleFunc("/api/automation/counters/increment", s.handleAutomationCounterIncrement)
	mux.HandleFunc("/api/automation/counters", s.handleAutomationCounters)
