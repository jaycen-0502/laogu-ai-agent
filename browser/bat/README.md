...
```powershell
pwsh -File bat/recover-profiles.ps1 -AppRoot 'E:\software\Laogu Browser'
```

确认结果后再写回 `app.db`：

```powershell
pwsh -File bat/recover-profiles.ps1 -AppRoot 'E:\software\Laogu Browser' -Apply
```

如果旧目录来自备份恢复，且怀疑存在跨内核残留状态，可同时为“风险目录”创建一份 `__repair_时间戳` 副本，再将新配置指向副本：

```powershell
pwsh -File bat/recover-profiles.ps1 -AppRoot 'E:\software\Laogu Browser' -Apply -RepairRisky
```
