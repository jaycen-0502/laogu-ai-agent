...
export interface AutomationActiveTask {
  taskId: string;
  profileId: string;
}
...
export async function fetchAutomationActiveTasks(): Promise<AutomationActiveTask[]> {
  const bindings: any = await getBindings();
  const goApp = (window as any).go?.main?.App;
  const raw = bindings?.AutomationTaskActiveList
    ? await bindings.AutomationTaskActiveList()
    : typeof goApp?.AutomationTaskActiveList === "function"
      ? await goApp.AutomationTaskActiveList()
      : [];
  ...
  return Array.isArray(raw)
    ? raw
        ...
        .filter((item: AutomationActiveTask) => item.taskId && item.profileId)
    : [];
}

export async function stopAutomationTasksByProfile(profileId: string): Promise<number> {
  ...
  const bindings: any = await getBindings();
  if (bindings?.AutomationTaskStopByProfile) {
    return Number(await bindings.AutomationTaskStopByProfile(normalizedProfileId)) || 0;
  }

  const goApp = (window as any).go?.main?.App;
  if (typeof goApp?.AutomationTaskStopByProfile === "function") {
    return Number(await goApp.AutomationTaskStopByProfile(normalizedProfileId)) || 0;
  }
  return 0;
}
