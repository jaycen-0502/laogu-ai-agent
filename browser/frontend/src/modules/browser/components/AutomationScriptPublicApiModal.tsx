...
export function AutomationScriptPublicApiModal({
  ...
  useEffect(() => {
    if (open) {
      if (selectedPrimaryTargetCode || selectedSecondaryTargetCode) {
        setSelectedTargetCodes([selectedPrimaryTargetCode, selectedSecondaryTargetCode].filter(Boolean));
      } else if (selectedTargetCode) {
        setSelectedTargetCodes([selectedTargetCode]);
      } else if (profiles.length > 0) {
        // 修正点：使用 launchCode 读取实例 Code，防止 TS2339 错误
        const defaultCode = profiles[0].launchCode || profiles[0].profileId || "";
        if (defaultCode) {
          setSelectedTargetCodes([defaultCode]);
        }
      }
    }
  }, [open, selectedTargetCode, selectedPrimaryTargetCode, selectedSecondaryTargetCode, profiles]);

  const targetCodeError =
    selectedTargetCodes.length > 0
      ? selectedTargetCodes.some((code) => !code.trim())
        ? "所有已选实例 Code 不能为空"
        : ""
  ...
  // 新增：多实例改变时，同时更新本地数组与 JSON 请求 Payload
  const handleMultiTargetCodesChange = (codes: string[]) => {
    setSelectedTargetCodes(codes);

    // 将选中的 Codes 数组注入到请求 Body 的 JSON 对象中
    try {
      const parsed = safeParseJSONObject(requestBodySource) || {};
      const updatedBody = {
        ...parsed,
        targetCodes: codes.filter(Boolean),
        // 如果数组只有 1 个，同步更新 targetCode 保持兼容
        targetCode: codes[0] || "",
      };
      updateConfig({
        requestBodyText: JSON.stringify(updatedBody, null, 2),
      });
    } catch {
      // 若解析失败，退回到更新单 Code
      if (codes[0]) {
        handleTargetCodeChange(codes[0]);
      }
    }
  };
  ...
  return (
    <AutomationScriptPublicApiModalView
      ...
      handleMultiTargetCodesChange={handleMultiTargetCodesChange}
