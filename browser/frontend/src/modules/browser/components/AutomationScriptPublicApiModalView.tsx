...
interface AutomationScriptPublicApiModalViewProps {
  ...
  // 新增：多实例数组处理逻辑
  handleMultiTargetCodesChange?: (codes: string[]) => void;
          ...
          <div className="rounded-xl border border-[var(--color-border-muted)] bg-[var(--color-bg-secondary)] px-3 py-3">
            <div className="flex items-center justify-between gap-2 mb-3">
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  handleMultiTargetCodesChange([...selectedTargetCodes, ""])
                }
                disabled={busy}
              >
                <Plus className="h-4 w-4" />
            ...
            {selectedTargetCodes.length > 0 ? (
              <div className="grid grid-cols-1 gap-3 xl:grid-cols-2">
                {selectedTargetCodes.map((code, idx) => (
                    ...
                    <div className="flex-1">
                      <AutomationInstanceSelector
                        ...
                        onCodeChange={(newCode) => {
                          const updated = [...selectedTargetCodes];
                          updated[idx] = newCode;
                          handleMultiTargetCodesChange(updated);
                        }}
                      />
                    </div>
                    {selectedTargetCodes.length > 1 && (
                      <Button
                        type="button"
                        variant="secondary"
                        size="sm"
                        onClick={() => {
                          const updated = selectedTargetCodes.filter(
                            (_, i) => i !== idx,
                          );
                          handleMultiTargetCodesChange(updated);
                        }}
