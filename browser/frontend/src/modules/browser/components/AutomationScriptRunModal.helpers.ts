...
export function parseProfileKeywordText(value: string): string[] {
  const result: string[] = [];
...
export function buildProfileKeywordParams(value: string): Record<string, unknown> {
  const keywords = parseProfileKeywordText(value);
...
export function mergeParamsTextWithProfileKeywords(
  paramsText: string,
  keywordText: string,
): string {
  const keywordParams = buildProfileKeywordParams(keywordText);
  if (Object.keys(keywordParams).length === 0) {
    return paramsText;
  }
  const normalized = paramsText.trim();
  const parsed = normalized ? JSON.parse(normalized) : {};
  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new Error("运行参数必须是 JSON 对象");
  }
  return JSON.stringify({ ...parsed, ...keywordParams }, null, 2);
}
