  ...
  if (!isUnlocked) {
    return createPortal(
      <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={onClose}>
        ...
        <div
          className="relative bg-[var(--color-bg-elevated)] border border-[var(--color-border)] rounded-xl shadow-2xl w-[360px] p-6 space-y-4 flex flex-col"
          onClick={e => e.stopPropagation()}
        >
          <button onClick={onClose} className="absolute top-3 right-3 text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] transition-colors">
            <X className="w-4 h-4" />
          </button>
          <div className="text-center space-y-1">
            <h3 className="text-base font-bold text-[var(--color-text-primary)]">🔒 切换代理验证</h3>
            <p className="text-xs text-[var(--color-text-muted)]">查看或修改代理配置请输入密码</p>
          </div>
          <input
            type="password"
            placeholder="请输入密码"
            ...
            autoFocus
            className="w-full px-3 py-2 border rounded-lg border-[var(--color-border)] bg-[var(--color-bg-input)] text-sm text-[var(--color-text-primary)] placeholder-[var(--color-text-muted)] focus:outline-none focus:border-[var(--color-primary)]"
          />
          <button
            onClick={handleVerifyPassword}
            className="w-full py-2 bg-[var(--color-primary)] hover:opacity-90 text-white font-medium text-sm rounded-lg transition-all"
          >
            解锁代理设置
          </button>
        </div>
