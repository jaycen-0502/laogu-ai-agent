...
import { AutomationAdminUnlockModal } from "../components/AutomationAdminUnlockModal";
import {
...
export function AutomationPage() {
  ...
  const requestAdminAction = (action: () => void) => {
    if (isAutomationAdminUnlocked()) {
      action();
      return;
    }
  ...
  const openScript = (scriptId: string) => {
    requestAdminAction(() => navigate(`/browser/automation/${scriptId}`));
  };
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      <AutomationPageHeader
        ...
        onCreate={() => requestAdminAction(() => setCreateOpen(true))}
        onExportSelected={() => requestAdminAction(() => void handleExportSelectedScripts())}
        onImport={() => requestAdminAction(() => setImportOpen(true))}
