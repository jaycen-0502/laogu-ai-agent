...
1. Laogu Browser 和 OpenClaw 在同一台机器上
2. Laogu Browser 的 LaunchServer 可访问，默认是 \`http://127.0.0.1:19876\`
3. OpenClaw 已安装 \`ant-chrome-openclaw\` skill，并且已有指向 Laogu Browser 的远程 CDP 浏览器配置
...
如果 Laogu Browser 开了 API Key，安装时补上对应参数即可。
...
| 自动化程序 | \`GET\` | \`/api/automation/scripts\` |
| 自动化程序 | \`GET\` | \`/api/automation/scripts/{scriptId}\` |
| 自动化程序 | \`POST\` | \`/api/automation/scripts/run\` |
| 自动化程序 | \`GET\` | \`/api/automation/scripts/runs\` |
