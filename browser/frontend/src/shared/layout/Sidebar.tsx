...
export function Sidebar() {
  ...
  // 💡 在这里填入需要屏蔽隐藏的侧边栏路径 (Path)
  const disabledPaths = ["/kernel", "/license", "/plugins"];
    ...
    >
      ...
      <nav className="flex-1 py-4 px-3 space-y-6 overflow-y-auto">
        {navigationConfig.map((section) => {
          // 过滤掉出现在 disabledPaths 中的菜单项
          const visibleItems = section.items.filter(
            (item) => !disabledPaths.includes(item.path)
          );

          // 如果当前分组中的菜单项都被删除了，则不渲染这个分组
