...
func (a *App) LicenseStatus() (backend.LicenseStatus, error) {
	return a.App.LicenseStatus()
}

func (a *App) LicenseRequestCode(regenerate bool) (backend.LicenseStatus, error) {
	return a.App.LicenseRequestCode(regenerate)
}

func (a *App) LicenseActivate(activationCode string) (backend.LicenseStatus, error) {
	return a.App.LicenseActivate(activationCode)
}
...
func main() {
	...
	err = wails.Run(&options.App{
		...
		OnShutdown: func(ctx context.Context) {
			...
			// 【新增这一行】：在彻底退出前，强行杀掉所有正在跑的 Playwright / 自动化程序！
			app.StopAllAutomationTasks()
