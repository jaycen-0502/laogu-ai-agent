...
- OpenClaw 调 Laogu Browser 的脚本执行接口
- Laogu Browser 在本地执行脚本
...
2. Laogu Browser 读取 `scriptId`
3. Laogu Browser 调 `AutomationScriptRunWithOptions`
4. 再进入 `RunScriptTask`
5. 本地 runner 执行脚本
...
这些都已经在 Laogu Browser 后端里了。

如果让 OpenClaw 直接跑：

- 会重复实现一套 runtime
- 会重复处理 LaunchServer 细节
- 会把脚本和 Laogu Browser 当前能力拆开
- 后面维护会很乱
...
也就是把 LaunchServer + CDP + Playwright 的细节都收进 Laogu Browser，不要让 OpenClaw 自己补：

1. OpenClaw 只给 `scriptId + selector + params`
2. Laogu Browser 在本地运行 `playwright-cdp` 脚本
...
如果 OpenClaw 想把 LaunchServer + Playwright + artifact 输出这些复杂度都交给 Laogu Browser，本地直接执行现成脚本，优先走：
