...
如果 Laogu Browser 开了 API Key，安装时补上 `-ApiKey` 或 `--api-key`。
...
这类任务会启动、停止、切换实例，或者执行已有自动化程序，属于有副作用的操作。不要指望它“自动猜到”你要不要切换目标实例，最好明确说清楚：
...
如果脚本已经在 Laogu Browser 里绑定了目标实例和默认参数，通常这两句就够了。
