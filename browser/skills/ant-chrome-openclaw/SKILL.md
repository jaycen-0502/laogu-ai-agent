...
description: Manually invoke when the user wants OpenClaw to work inside a local Laogu Browser / ant-chrome instance through LaunchServer and remote CDP, including launching by launchCode, checking or switching the active runtime target, stopping an instance, or running a saved Laogu Browser automation script.
when_to_use: Use this skill whenever the task mentions Laogu Browser, ant-chrome, LaunchServer, launchCode, profileId/profileName selectors, runtime active/status/stop, attaching OpenClaw to an already running browser via remote CDP, or executing a saved Laogu Browser automation script such as news-query-txt. This workflow can launch, stop, or mutate browser state, so prefer explicit invocation instead of silent automatic use.
compatibility: Designed for OpenClaw with same-host Laogu Browser LaunchServer access. Requires loopback access to the LaunchServer base URL plus either curl or PowerShell for host-side HTTP calls.
...
# Laogu Browser via OpenClaw

Invoke this skill explicitly when the user wants OpenClaw to control browser work that should run inside Laogu Browser (`ant-chrome`) instead of a browser launched directly by OpenClaw.
...
- OpenClaw Gateway and Laogu Browser run on the same machine.
- Laogu Browser LaunchServer is reachable on loopback.
- OpenClaw Browser has a preconfigured remote CDP profile pointing at the Laogu Browser LaunchServer URL.
...
- Treat Laogu Browser as the source of truth for profile state, proxies, tags, launch codes, and Chrome cores.
- Do not switch to another Laogu Browser target while the current remote CDP session is still being used unless the user explicitly asks to switch. LaunchServer exposes one active CDP target at a time. Query `GET /api/runtime/active` before switching if the current target matters.
- Do not claim that OpenClaw `browser stop` stops the Laogu Browser process. For remote CDP profiles it only detaches the OpenClaw control session.
