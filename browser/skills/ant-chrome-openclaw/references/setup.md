...
- The bridge between them is Laogu Browser LaunchServer plus a remote CDP browser profile in OpenClaw.

## Preconditions

1. Laogu Browser must be running on the same machine as the OpenClaw Gateway.
2. Laogu Browser LaunchServer should use a fixed port for stability. The project default is `19876`.
3. If API auth is enabled in Laogu Browser, keep the header at `X-Ant-Api-Key` unless you have a reason to change it.
4. OpenClaw Browser must be enabled.

## Laogu Browser config

Laogu Browser already defaults to a fixed LaunchServer port:
...
Use a remote CDP browser profile that points to the Laogu Browser LaunchServer URL.
...
- preserve existing config fields outside the Laogu Browser related sections
...
If Laogu Browser auth is enabled, add either:
...
- `browser stop` in OpenClaw only detaches from remote CDP. It does not stop the Laogu Browser instance itself.
