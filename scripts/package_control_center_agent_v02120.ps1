...
$version = "0.21.20"
$release = Join-Path $root "release\2026-08-28-control-center-agent-startup-queue-v0.21.20"
$payload = Join-Path $release "01-windows-portable\Laogu-Control-Center-Agent-0.21.20"
$checks = Join-Path $release "02-checksums"
$docs = Join-Path $release "03-build-record"
$zip = Join-Path $release "01-windows-portable\Laogu-Control-Center-Agent-0.21.20-windows-amd64.zip"
...
@(
    ...
    ""
    "This build serializes only Profile startup and CDP readiness."
    "Different ready Profiles can continue running automation concurrently."
    ""
...
@(
    ...
    "Change: serialized Profile startup, CDP readiness polling, per-Profile task lock and controller status logs"
    "Verification: 34 startup/task regression tests passed; 43 Agent/Desktop tests passed; py_compile and diff checks passed"
    "Unchanged: automation script content, Laogu Browser kernel, server and Web backend"
