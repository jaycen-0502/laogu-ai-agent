    ...
    *,
    ...
    def control_overview(
        ...
        for item in today_metrics:
            key = (item.agent_id, item.profile_id)
            aggregate = metrics_by_profile.setdefault(key, empty_metrics())
        ...
        for profile in sorted(profiles, key=lambda item: item.profile_id):
            ...
            payload |= {
                ...
                "today_metrics": metrics_by_profile.get(
                    (profile.agent_id, profile.profile_id), empty_metrics()
                ),
            }
