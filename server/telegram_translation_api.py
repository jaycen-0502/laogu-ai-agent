...
def register_telegram_translation_routes(app: FastAPI, *, get_db: Callable, current_user: Callable, cipher: CredentialCipher) -> None:
    ...
    def get_binding(request: Request, workspace_id: str = "", user: User = Depends(current_user), db: Session = Depends(get_db)):
        ...
        return _binding_dict(db.scalar(query))
    ...
    def update_binding(body: TelegramBindingUpdate, request: Request, user: User = Depends(current_user), db: Session = Depends(get_db)):
        ...
        item.updated_at = now()
        db.commit()
        admin_allowed = db.scalar(select(TelegramAllowedUser).where(TelegramAllowedUser.binding_id == item.id, TelegramAllowedUser.telegram_user_id == body.admin_telegram_user_id))
        if admin_allowed:
            admin_allowed.enabled = True
            admin_allowed.display_name = admin_allowed.display_name or "管理员"
            admin_allowed.updated_at = now()
        else:
            db.add(TelegramAllowedUser(binding_id=item.id, telegram_user_id=body.admin_telegram_user_id, display_name="管理员"))
        db.commit()
