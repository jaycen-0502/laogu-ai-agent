...
from tests.test_ai_providers import FERNET_KEY, auth, make_env
...
def test_admin_binds_encrypted_bot_and_manages_allowlist():
    ...
    with env["client"].app.state.SessionLocal() as db:
        binding = db.scalar(
            select(TelegramBotBinding).where(
                TelegramBotBinding.workspace_id == env["workspace_id"]
            )
        )
        assert binding is not None
        assert binding.bot_token_encrypted != BOT_TOKEN
        assert env["client"].app.state.telegram_manager.cipher.decrypt(
            binding.bot_token_encrypted
        ) == BOT_TOKEN
        admin_allowed = db.scalar(
            select(TelegramAllowedUser).where(
                TelegramAllowedUser.binding_id == binding.id,
                TelegramAllowedUser.telegram_user_id == "10001",
            )
        )
        assert admin_allowed is not None
    ...
    users = env["client"].get(
        f"/api/admin/telegram/translation/users?workspace_id={env['workspace_id']}",
        headers=auth(env["admin"]),
    )
    assert {item["telegram_user_id"] for item in users.json()} == {
        "10001",
        "10002",
    }
    removed = env["client"].delete(
        f"/api/admin/telegram/translation/users/10002?workspace_id={env['workspace_id']}",
        headers=auth(env["admin"]),
    )
