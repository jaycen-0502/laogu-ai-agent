...
export function ControlCenterPage() {
  ...
  return <>
    ...
    <section className="panel"><div className="section-heading"><h2>Profile / Account</h2><span className="muted">点击一行查看任务和活动详情</span></div>
       {data.profiles.length ? <div className="table-wrap"><table><thead><tr><th>Profile</th><th>账号</th><th>代理节点</th><th>今日执行</th><th>Agent</th><th>浏览器</th><th>登录</th><th>当前任务</th><th>最近状态</th></tr></thead><tbody>{data.profiles.map((profile) => <tr className="clickable" key={profile.profile_record_id} onClick={() => void openProfile(profile.profile_record_id)}>
         ...
         <td>{profile.proxy_name || profile.proxy_id || "直连/未配置"}<div className="muted mono">{profile.proxy_host ? `${profile.proxy_host}${profile.proxy_port ? `:${profile.proxy_port}` : ""}` : "-"}</div></td>
    ...
    {detail && <div className="control-detail-backdrop" onClick={() => setDetail(null)}><section className="panel control-detail" onClick={(event) => event.stopPropagation()}>
      <div className="section-heading"><div><h2>{detail.profile.x_username || detail.profile.profile_id}</h2><p className="muted">{detail.profile.profile_id} · {detail.agent?.agent_name || detail.profile.agent_id}</p></div><button onClick={() => setDetail(null)}>关闭</button></div>
       <div className="control-detail-summary"><span>浏览器 <State value={detail.profile.browser_status} /></span><span>登录 <State value={detail.profile.login_status} /></span><span>账号 <State value={detail.profile.account_status} /></span><span>代理 <strong>{detail.profile.proxy_name || detail.profile.proxy_id || "直连/未配置"}</strong></span><span>任务 <strong>{detail.tasks.length}</strong></span></div>
